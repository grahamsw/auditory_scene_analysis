...
const isSpatialSwapped = ref(false)
...
const set2 = ref<HomophonicSettings>({ ...DEFAULT_SETTINGS })
...
function saveToSet(index: number) {
  const settings: HomophonicSettings = {
    ...
    isSpatialSwapped: isSpatialSwapped.value,
...
function loadFromSet(index: number) {
  ...
  isSpatialSwapped.value = settings.isSpatialSwapped
...
function updatePanning() {
  ...
  const constantPan = isSpatialSwapped.value ? 1 : -1
  const intermittentPan = isSpatialSwapped.value ? -1 : 1
...
<template>
  <div class="controls">
    ...
    <fieldset>
      <legend>Spatial Mapping</legend>
      <div class="preset-controls">
        <span class="preset-label">
          {{ isSpatialSwapped ? 'Swapped' : 'Normal' }}
        </span>
        <button class="small" @click="isSpatialSwapped = !isSpatialSwapped">
          Swap Channels
        </button>
      </div>
      <div style="text-align: center; font-size: 0.8rem; color: #888; margin-top: 0.5rem;">
        Constant: {{ isSpatialSwapped ? 'Right' : 'Left' }} | 
        Intermittent: {{ isSpatialSwapped ? 'Left' : 'Right' }}
      </div>
    </fieldset>
